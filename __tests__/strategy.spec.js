...
describe('Strategy Tests', function () {
  ...
  it('should fetch the user profile', function() {
    const done = vi.fn();
    vi.spyOn(onshapeStrategy._oauth2, 'get').mockImplementation(function(url, token, cb) {
      const response = {id: 'AUserId', name: 'User Name',html_url: 'http://example.com', email: 'user@example.com'};
  ...
  it('should handle an error when fetching the user profile', function() {
    ...
    onshapeStrategy.userProfile('AnAccessToken', done);
    expect(onshapeStrategy._oauth2.get).toHaveBeenCalledWith(expect.any(String), 'AnAccessToken', expect.any(Function));
    expect(done).toHaveBeenLastCalledWith( {message: 'Failed to fetch user profile', name: 'InternalOAuthError', oauthError: 'An Error occurred'} );
  });
